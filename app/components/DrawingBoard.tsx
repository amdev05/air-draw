...
interface HandLandmarkerResult { landmarks: HandLandmark[][]; }
...
const COLORS = [
  ...
  "#7FFF00", "#00FF80", "#00FFFF", "#007FFF",
  "#8B00FF", "#FF00FF", "#FFFFFF", "#1a1a1a",
];
...
export default function DrawingBoard() {
  ...
  const pickColor = (c: string) => { colorRef.current = c; setColor(c); };
  const pickSize = (s: number) => { sizeRef.current = s; setSize(s); };
  ...
  useEffect(() => {
    ...
    const resize = () => {
      [drawRef, overlayRef].forEach(r => {
        if (r.current) { r.current.width = innerWidth; r.current.height = innerHeight; }
      });
    };
    resize();
    ...
    (async () => {
      try {
        ...
        const tick = () => {
          ...
          if (!dc || !oc) { rafRef.current = requestAnimationFrame(tick); return; }
          ...
          if (vid.readyState >= 2 && vid.currentTime !== lastT) {
            ...
            try { res = hlRef.current?.detectForVideo(vid, performance.now()) ?? null; } catch { /* skip */ }
          }
          ...
          if (res?.landmarks?.length) {
            ...
            let g: DrawMode = "idle";
            if (iUp && mUp && rUp && pUp) g = "erase";
            else if (iUp && !mUp && !rUp && !pUp) g = "draw"; // removed !tUp — thumb state ignored
            else if (tUp && !iUp && !mUp && !rUp && !pUp) g = "undo";
            else if (pUp && !iUp && !mUp && !rUp) g = "redo"; // removed !tUp
            setMode(g);
            ...
            if (g === "undo" || g === "redo") {
              if (holdGestRef.current !== g) {
                holdGestRef.current = g; holdStartRef.current = performance.now(); holdFiredRef.current = false;
                ...
                octx.lineWidth = 4; octx.lineCap = "round"; octx.stroke();
                if (prog >= 1) { holdFiredRef.current = true; if (g === "undo") doUndo(); else doRedo(); }
              }
            ...
            if (g === "draw" && !inToolbar) {
              ...
              prevPtRef.current = { x: tipX, y: tipY };
              octx.beginPath();
              octx.arc(tipX, tipY, sizeRef.current / 2 + 8, 0, Math.PI * 2);
              octx.fillStyle = colorRef.current + "40"; octx.fill();
              octx.strokeStyle = colorRef.current; octx.lineWidth = 3; octx.stroke();
              ...
              dctx.beginPath(); dctx.arc(palmX, palmY, ERASER_R, 0, Math.PI * 2);
              dctx.fillStyle = "rgba(0,0,0,1)"; dctx.fill();
              dctx.restore();
              octx.beginPath(); octx.arc(palmX, palmY, ERASER_R, 0, Math.PI * 2);
              octx.strokeStyle = "rgba(255,80,80,0.9)"; octx.lineWidth = 3; octx.stroke();
              octx.fillStyle = "rgba(255,80,80,0.12)"; octx.fill();
              ...
              octx.beginPath(); octx.arc(tipX, tipY, 8, 0, Math.PI * 2);
              octx.fillStyle = "rgba(255,255,255,0.3)"; octx.fill();
            }
            ...
            if (g === "draw" && inToolbar) {
              ...
              if (act) {
                if (act !== dwellActRef.current) {
                  dwellActRef.current = act; dwellStartRef.current = performance.now();
                  dwellCoolRef.current = false; setDwellBtn(act);
                  ...
                  octx.strokeStyle = "white"; octx.lineWidth = 3; octx.lineCap = "round"; octx.stroke();
                  if (prog >= 1) {
                    dwellCoolRef.current = true; triggerBtn(act); setDwellBtn(null);
                    setTimeout(() => { dwellActRef.current = null; dwellCoolRef.current = false; }, 500);
                  }
                }
              } else if (dwellActRef.current) { dwellActRef.current = null; setDwellBtn(null); }
              octx.beginPath(); octx.arc(tipX, tipY, 10, 0, Math.PI * 2);
            ...
            lostDrawFramesRef.current++;
            if (lostDrawFramesRef.current >= MAX_LOST_FRAMES) {
              setHasHand(false); setMode("idle");
              prevPtRef.current = null; prevModeRef.current = "idle";
              holdGestRef.current = null;
              if (dwellActRef.current) { dwellActRef.current = null; setDwellBtn(null); }
            }
          }
        ...
        if (!cancelled) { setErr(e.message || "Initialization failed"); setLoading(false); }
      }
    })();

    return () => {
      cancelled = true; runRef.current = false;
  ...
  const modeLabel = {
    ...
    undo:  "↩️  Undo…",
    redo:  "↪️  Redo…",
    idle:  hasHand ? "✋  Raise 1 finger to draw" : "👋  Show your hand",
  }[mode];
      ...
      {!loading && !err && (
        <>
          ...
          <div className="absolute top-5 left-1/2 -translate-x-1/2 z-40 pointer-events-none">
            <div className={`px-5 py-2 rounded-full text-sm font-semibold tracking-wide backdrop-blur-md border transition-all duration-300 ${modeBadge}`}>
              {modeLabel}
            </div>
          </div>
          ...
          <div className="absolute top-5 left-5 z-40 space-y-1.5 text-xs text-white/30 pointer-events-none select-none">
            <p>☝️  1 finger → Draw</p>
            <p>🤚  Open palm → Erase</p>
            <p>✌️  2 fingers → Pause</p>
            <p>👍  Thumb (hold) → Undo</p>
            <p>🤙  Pinky (hold) → Redo</p>
            <p className="mt-2 text-white/20">Point at toolbar to select</p>
          </div>
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-40 flex items-center gap-3 px-5 py-3.5 rounded-2xl bg-black/50 backdrop-blur-xl border border-white/10 shadow-2xl">
              ...
              {BRUSH_SIZES.map((s) => (
                ...
                >
                  <span
                    ...
                    style={{ width: Math.min(s * 0.85, 22), height: Math.min(s * 0.85, 22), backgroundColor: color }}
                  />
                </button>
              ))}
            </div>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                dwellBtn === "undo" ? "bg-amber-500/20 text-amber-300" : "text-white/60 hover:text-white hover:bg-white/10"
              }`}
            >↩ Undo</button>
            ...
            <button
              ...
              onClick={doRedo}
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                dwellBtn === "redo" ? "bg-sky-500/20 text-sky-300" : "text-white/60 hover:text-white hover:bg-white/10"
              }`}
            >↪ Redo</button>
            ...
            <button
              ...
              onClick={doClear}
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                dwellBtn === "clear" ? "bg-red-500/20 text-red-300" : "text-red-400/70 hover:text-red-300 hover:bg-red-500/15"
              }`}
            >🗑 Clear</button>
          </div>
        </>
